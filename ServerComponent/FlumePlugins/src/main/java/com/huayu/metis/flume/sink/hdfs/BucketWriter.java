...
class BucketWriter {
    ...
    BucketWriter(long rollInterval, long rollSize, long rollCount, long batchSize,
                 ...
                 String inUseSuffix, String fileSuffix, HDFSWriter writer,
                 ScheduledExecutorService timedRollerPool, UserGroupInformation user,
                 SinkCounter sinkCounter, int idleTimeout, HDFSEventSink.WriterCallback onCloseCallback,
                 String onCloseCallbackPath, long callTimeout,
                 ExecutorService callTimeoutPool, int maxRotations) {
        ...
        this.maxConsecUnderReplRotations = maxRotations;
            ...
            if (idleTimeout > 0) {
                ...
                if (idleFuture == null || idleFuture.cancel(false)) {
                    Callable<Void> idleAction = new Callable<Void>() {
                        public Void call() throws Exception {
                            LOG.info("Closing idle bucketWriter {} at {}", bucketPath, System.currentTimeMillis());
                            if (isOpen) {
                                close(true);
                            }
                            return null;
    ...
    public synchronized void append(final Event event) throws IOException, InterruptedException {
        ...
        try {
            ...
            LOG.warn("Caught IOException writing to HDFSWriter ({}). Closing file (" + bucketPath + ") and rethrowing exception.",
                    e.getMessage());
            try {
                close(true);
